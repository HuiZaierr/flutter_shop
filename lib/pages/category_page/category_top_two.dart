...
class CategoryTopTwo extends StatelessWidget {
  bool isClick = false;
  @override
  Widget build(BuildContext context) {
    return Provide<StatusProvide>(builder: (context, child, data) {
      return Container(
        ...
        child: Wrap(
          spacing:7,
          children: widgetList(data.statusList, isClick),
        ),
      );
    });
  }
...
widgetList(List newList,bool isClick){
  if (newList.length > 0) {
    ...
    for (var i = 0; i < newList.length; i++) {
      isClick = Provide.value<StatusProvide>(Application.context).id == newList[i].id ? true : false;
